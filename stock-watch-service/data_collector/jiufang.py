...
from config import DATA_SOURCES, STOCK_FILTER, DATA_CACHE_DIR, PERFORMANCE, REQUEST_TIMEOUT, PERFORMANCE, MAX_STOCK_PAGES, LLM_CONFIG_SILICON, LLM_CONFIG_DASHSCOPE  # 新增这两个
...
class JiuFangCollector:
    ...
    def _fetch_stock_page(self, sort_type: str, page: int) -> List[Tuple]:
        ...
        try:
            timeout = REQUEST_TIMEOUT  # 从config导入
            resp = self.session.get(API_BASE, params=params, headers=headers, timeout=timeout)            
            data = resp.json()
